...
### 8.4 Nível 3 — Trilha A (multiagente): fiz, e a hipótese não se confirmou

Ver seção 10 abaixo. Implementei, medi, e o resultado foi contra o que eu esperava.
...
## 10. Nível 3 — o que eu esperava e o que aconteceu

Implementei a Trilha A: três papéis encadeados (Triador, Investigador, Redator), estado
compartilhado num dataclass e condição de parada em três pontos. Está em `nivel_3/fluxo.py`,
com o diagrama em `docs/ARQUITETURA.md`.

Escolhi a A por dois motivos. O primeiro é reuso: o Investigador usa o `nivel_2/tools.py`
sem nenhuma alteração e o Redator usa o mesmo `ParecerPLD` do Nível 1 — o fluxo novo não
duplicou lógica nenhuma. O segundo motivo era um problema de custo que eu tinha medido no
Nível 2: lá, todo cliente recebe investigação completa, inclusive os 4 de 6 que o próprio
agente concluiu como risco baixo. A ideia era que o Triador arquivasse esses casos barato.

### O que eu tinha escrito antes de rodar

Que ia comparar custo e concordância contra o agente único, e que **se o multiagente
gastasse mais tokens pra chegar nos mesmos pareceres, a arquitetura não se pagava, e eu
diria isso em vez de esconder.**

Foi o que aconteceu.

### O Triador arquivou zero em cinco

Rodei sobre 5 clientes. Nenhum foi arquivado. Ele mandou investigar todos, sempre com o
mesmo tipo de motivo: "valor atípico em 2 operações", "valor atípico em 3 operações".

Isso não é bug — é o Triador sendo conservador. Diante de "a regra foi acionada", ele nunca
acha que tem informação suficiente pra descartar. E, pensando bem, faz sentido: eu dei a ele
só o resumo do caso, sem ferramenta nenhuma, justamente pra ser barato. Mas com pouca
informação, a decisão segura é sempre investigar.

**Ou seja, o que torna o Triador barato é o mesmo que o impede de arquivar.** Não tinha
percebido essa tensão quando desenhei.

### Sem arquivamento, a economia não existe

| Cliente | Agente único (Nível 2) | Multiagente (Nível 3) |
|---|---|---|
| CLI-014 | ~13.600 tokens | 8.365 |
| CLI-028 | ~7.100 | 9.197 |
| CLI-013 | falhou por quota | 8.031 |
| CLI-005 | ~5.600 | 8.277 |

O custo ficou parecido, e em dois dos três casos comparáveis ficou **maior** — com três
chamadas em vez de uma e mais pontos de falha. Dos 5 clientes, 1 falhou por erro de API no
meio do fluxo (o tratamento de erro funcionou e não derrubou os outros, mas é um ponto de
falha que o agente único não tem).

### O problema que eu não esperava: gargalo de informação

Esse me pareceu mais grave que o custo.

Olhando a coluna `n_evidencias` dos casos concluídos: três deles registraram **uma única
evidência**. O `CLI-005` registrou 11 e foi o único que saiu como risco médio, com 3 red
flags. Os outros três, com uma evidência cada, saíram como baixo sem nenhuma red flag.

O Redator só sabe o que o Investigador escreveu. Se a evidência não for registrada, ela
deixa de existir pra quem decide.

O caso mais claro é o `CLI-029`, que eu rodei separado. O agente único do Nível 2 concluiu
**baixo**, com um argumento forte: havia um saque, dois depósitos e uma transferência
recebida no mesmo dia, ou seja, dinheiro nas duas direções, o que não é fracionamento. O
fluxo multiagente concluiu **médio** — porque o Investigador registrou nove evidências mas
**não incluiu os tipos das operações**, que era exatamente o argumento decisivo.

E o Redator, sem essa informação, escreveu como red flag: *"Regra de fracionamento acionada
em 2026-05-26"*. Isso é circular — ele repetiu a regra em vez de avaliá-la, porque não
recebeu evidência suficiente pra fazer outra coisa.

**No agente único isso não acontece, porque quem investiga é quem conclui.** Separar papéis
torna o parecer rastreável — cada red flag vem de uma evidência registrada, que veio de uma
ferramenta — mas cria um funil onde informação se perde na passagem.

### O que eu concluo disso

A arquitetura multiagente, do jeito que eu montei, **não se justifica pra este problema**.
Ela custa igual ou mais, tem mais pontos de falha, e perde informação entre os papéis.

O que eu tentaria se tivesse mais tempo, em ordem:

1. **Dar uma ferramenta ao Triador.** Se ele pudesse consultar o `historico_cliente` antes
   de decidir, talvez arquivasse casos que hoje passa adiante. O custo dele subiria, mas a
   economia viria dos casos evitados. É medível: bastaria comparar a taxa de arquivamento.

2. **Passar o estado bruto, não só as evidências.** Em vez de o Redator receber só a lista
   que o Investigador escreveu, ele receberia também os retornos das ferramentas. Isso
   fecha o gargalo, mas aumenta o custo de entrada do Redator — outro trade-off pra medir.

3. **Exigir cobertura mínima do Investigador.** Uma evidência só é pouco. Poderia validar
   que ele registrou pelo menos um fato que sustenta e um que enfraquece a suspeita, que é
   o que o prompt já pede mas não verifica.

Mas registro o principal: **a hipótese que motivou a arquitetura não se confirmou na
medição, e isso é um resultado, não um fracasso.** Se eu tivesse ajustado o Triador até ele
arquivar alguma coisa só pra mostrar economia, estaria medindo o meu ajuste, não a
arquitetura — o mesmo erro que evitei no confronto do Nível 2.
